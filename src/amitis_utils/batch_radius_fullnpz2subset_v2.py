...
dt = 0.005  # simulation dt defined in Amitis.inp [seconds]
select_R = 2480.e3  # shell within which you want to keep particles [m]

outdir = f"/Volumes/data_backup/mercury/extreme/CPN_BNBV_extendedX/precipitation_timeseries/"
os.makedirs(outdir, exist_ok=True)

def build_file_path(pfix: str) -> list:
    input_folder = "/Volumes/data_backup/mercury/extreme/CPN_BNBV_extendedX/subset/"
...
for sim_step in sim_steps:
    prefix = f"Subset_CPN_BNBV_" + "%06d" % sim_step
    ...
    outfile_npz = os.path.join(outdir, f"CPN_BNBV_{"%06d" % sim_step}_precipitation_shell.npz")
