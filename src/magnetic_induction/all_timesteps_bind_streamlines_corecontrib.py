...
def B_from_J_poisson_lowmem(x_km, y_km, z_km, Jx_nA, Jy_nA, Jz_nA):
    ...
    M = spla.LinearOperator((N, N), matvec=Minv)

    def solve(J):
        b = (MU0 * J).ravel()
        sol, _ = spla.cg(Aop, b, maxiter=10000, M=M)
...
nrows = len(steps)
fig_stream, ax_stream = plt.subplots(nrows, 3, figsize=(15, 4*nrows), constrained_layout=True)
fig_parallel, ax_parallel = plt.subplots(nrows, 3, figsize=(15, 4*nrows), constrained_layout=True)
...
for row, step in enumerate(steps):
    ...
    # rotated timestep label
    ax_stream[row, 0].text(-0.28, 0.5, f"t={step*0.002:.1f}s",
                            transform=ax_stream[row, 0].transAxes,
                            rotation=90, va="center", ha="center", fontsize=12)

    ax_parallel[row, 0].text(-0.28, 0.5, f"t={step*0.002:.1f}s",
                              transform=ax_parallel[row, 0].transAxes,
                              rotation=90, va="center", ha="center", fontsize=12)
...
fig_stream.savefig(os.path.join(output_folder, "Bind_streamlines_timesteps.png"), dpi=300)
fig_parallel.savefig(os.path.join(output_folder, "Bind_parallel_timesteps.png"), dpi=300)
