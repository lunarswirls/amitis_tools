...
compute_twist = True
compute_open_twist = True  # NEW: Enable twist calculation for open field lines
...
if plot_3d_category:
    ...
    twist_labels = {
        ...
        'moderate': f'Moderate ({moderate_twist:.1f}-{critical_twist:.2f} turns, 2π rad)',
        'critical': f'Critical ({critical_twist:.2f}-{high_twist:.1f} turns, kink instability)',
    ...
    if len(all_twisted) > 0:
        # Enforce ordering: low > moderate > critical > high
        twist_order = ['low', 'moderate', 'critical', 'high']
        ...
        print(f"\nTwist statistics for 3D plot:")
        print(f"  Twisted closed field lines: {len(trajectories_3d['twisted_closed'])}")
        for level in ['low', 'moderate', 'critical', 'high']:
...
if (compute_twist or compute_open_twist) and len(fieldline_twist_data) > 0:
    ...
    if compute_twist:
        ...
        print(f"  Critical twist: {len(twisted_flux_tubes_closed['critical'])}")
    ...
    if compute_open_twist:
        ...
        print(f"  Critical twist: {len(twisted_flux_tubes_open['critical'])}")
