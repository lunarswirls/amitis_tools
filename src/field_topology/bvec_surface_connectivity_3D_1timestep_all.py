...
for case in cases:
    ...
    output_folder = f"/Users/danywaller/Projects/mercury/extreme/bfield_topology_new2RM/"
    ...
    for seed in seeds:
        traj_fwd, exit_fwd_y = trace_field_line_rk(seed, Bx, By, Bz, x, y, z, RM, max_steps=max_steps, h=h_step, surface_tol=1)
        traj_bwd, exit_bwd_y = trace_field_line_rk(seed, Bx, By, Bz, x, y, z, RM, max_steps=max_steps, h=-h_step, surface_tol=1)
    ...
    if make_plots:
        ...
        plt.savefig(os.path.join(output_folder,f"{case}_field_topology_{step}.png"), dpi=150, bbox_inches="tight")
        print("Saved:\t", os.path.join(output_folder,f"{case}_field_topology_{step}.png"))
        plt.close()
        ...
        plt.savefig(os.path.join(output_folder, f"{case}_field_footprints_{step}.png"), dpi=150,
                    bbox_inches="tight")
        print("Saved:\t", os.path.join(output_folder, f"{case}_field_footprints_{step}.png"))
        plt.close()
    ...
    print(f"Saved OCB curve to:\t{ocb_csv}")
    ...
    print(f"Saved connectedness metrics to:\t{metrics_csv}\n\n")
