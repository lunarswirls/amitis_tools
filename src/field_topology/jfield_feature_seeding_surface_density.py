...
output_folder = f"/Users/danywaller/Projects/mercury/extreme/jfield_topology_density_seeding_{rmin}-{rmax}_RM/{case}/"
...
def load_field(nc_file):
    ...
    J_x = np.transpose(ds["Jx"].isel(time=0).values, (2,1,0))
    J_y = np.transpose(ds["Jy"].isel(time=0).values, (2,1,0))
    J_z = np.transpose(ds["Jz"].isel(time=0).values, (2,1,0))
...
jtime = datetime.now()
print(f"Loaded J-vector and density at {str(jtime)}")
...
out_html = f"{case}_{step}_J_vector_density_seed_topology.html"
