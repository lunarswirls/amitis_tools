...
output_folder = f"/Users/danywaller/Projects/mercury/extreme/timeseries_surface_flux/{case}"
...
for sim_step in sim_steps:
    ...
    # Plot
    # -------------------------------
    fig, (ax0, ax1, ax2) = plt.subplots(1, 3, figsize=(16, 4), subplot_kw={"projection": "hammer"})
    ...
    # total density
    # ax0 = axes[0]
    sc = ax0.pcolormesh(lon_grid, lat_grid, tot_den, cmap="plasma", shading="auto", vmin=0, vmax=50)
    ...
    # v_r
    # ax1 = axes[1]
    sc = ax1.pcolormesh(lon_grid, lat_grid, v_r, cmap="RdBu_r", shading="auto", vmin=-150, vmax=150)
    ...
    sc = ax2.pcolormesh(lon_grid, lat_grid, log_flux_surface, cmap="viridis", shading="auto", vmin=6, vmax=8)
    cbar = fig.colorbar(sc, ax=ax2, orientation="horizontal", pad=0.05, shrink=0.5)
    cbar.set_label(r"$\log_{10}$(F [cm$^{-2}$ s$^{-1}$])")
    ...
    plt.suptitle(f"{case.replace("_"," ")} at t = {sim_step * 0.002} s", y=0.95)
    plt.tight_layout()
    # Save figure
    plt.tight_layout()
    outfile_png = os.path.join(output_folder, f"radial_flux_calc_testing_{case}_{filetime}.png")
