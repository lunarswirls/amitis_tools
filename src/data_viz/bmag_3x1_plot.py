...
out_folder = f"/Users/danywaller/Projects/mercury/extreme/timeseries_bmag_{slice_tag}/{case}/"
...
def extract_slice_fields(ds: xr.Dataset, use_slice: str):
    ...
    Bx = ds["Bx_tot"].sel(**sel_kw, method="nearest").squeeze()
    By = ds["By_tot"].sel(**sel_kw, method="nearest").squeeze()
    Bz = ds["Bz_tot"].sel(**sel_kw, method="nearest").squeeze()
...
for sim_step in sim_steps:
    ...
    for ax, use_slice in zip(axes, use_slices):
        ...
        bmag = extract_slice_fields(ds, use_slice)

        ds.close()

        circle = plt.Circle((0, 0), 1, edgecolor='black', facecolor='cornflowerblue', alpha=0.3, linewidth=1, )
        last_im = ax.pcolormesh(x_plot, y_plot, bmag, shading='auto', cmap='Greens', vmin=0, vmax=c_max)
        ax.add_patch(circle)
    ...
    if last_im is not None:
        ...
        cbar.set_label(r"|B| [nT]")

    fig.suptitle(rf"{case.replace("_"," ")} |B| at t = {sim_step * 0.002:.3f} s", fontsize=18, y=0.99)
    # plt.tight_layout()
    fig_path = os.path.join(out_folder, f"{case}_bmag_{sim_step}.png")
    plt.savefig(fig_path, dpi=300, bbox_inches='tight')
    plt.close()

print(f"Done plotting |B| to {out_folder}")
