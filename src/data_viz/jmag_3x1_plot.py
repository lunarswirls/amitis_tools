...
out_folder = f"/Users/danywaller/Projects/mercury/extreme/timeseries_jmag_{slice_tag}/{case}/"
...
for sim_step in sim_steps:
    ...
    for ax, use_slice in zip(axes, use_slices):
        ...
        jmag = extract_slice_fields(ds, use_slice)

        ds.close()

        circle = plt.Circle((0, 0), 1, edgecolor='black', facecolor='cornflowerblue', alpha=0.3, linewidth=1, )
        last_im = ax.pcolormesh(x_plot, y_plot, jmag, shading='auto', cmap='RdPu', vmin=0, vmax=c_max)
        ax.add_patch(circle)
    ...
    if last_im is not None:
        ...
        cbar.set_label(r"|J| [nA/m$^2$]")

    fig.suptitle(rf"{case.replace("_"," ")} |J| at t = {sim_step * 0.002:.3f} s", fontsize=18, y=0.99)
    # plt.tight_layout()
    fig_path = os.path.join(out_folder, f"{case}_jmag_{sim_step}.png")
    plt.savefig(fig_path, dpi=300, bbox_inches='tight')
    plt.close()

print(f"Done plotting |J| to {out_folder}")
