...
if "Base" in case:
    ...
    if transient and not post_transient and not new_state:
        ...
        sim_steps = range(340000, 350000 + 1, 1000)  # new state
    else:
...
for sim_step in sim_steps:
    ...
    if "Base" in case:
        outpath = os.path.join(out_folder_ts, f"{case}_{plot_id.lower()}_boundaries_{slice_tag}_{sim_step:06d}_pre-transient.png")
    elif "HNHV" in case:
        if transient and not post_transient and not new_state:
            outpath = os.path.join(out_folder_ts, f"{case}_{plot_id.lower()}_boundaries_{slice_tag}_{sim_step:06d}_transient.png")
        elif post_transient and not transient and not new_state:
            outpath = os.path.join(out_folder_ts, f"{case}_{plot_id.lower()}_boundaries_{slice_tag}_{sim_step:06d}_post-transient.png")
        elif new_state and not post_transient and not transient:
            outpath = os.path.join(out_folder_ts, f"{case}_{plot_id.lower()}_boundaries_{slice_tag}_{sim_step:06d}_newstate.png")
